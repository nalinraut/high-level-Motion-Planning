...
void MonteCarloNDTest(int d,int num)
{
  ...
  for(int iters=0;iters<num;iters++) {
    ...
    res=ramp.SolveMinTimeLinear(amax,vmax);
    assert(res);
  }
  printf("\n");
  printf("Success rate: %g min time, %g min accel\n",Real(numSolvedMinTime)/Real(num),Real(numSolvedMinAccel)/Real(num));
...
{
  printf("Testing ParabolicRamp min-accel failures...\n");
  TestRamps("PP_SolveMinAccel_failure.dat");
  TestRamps("PLP_SolveMinAccel_failure.dat");
  TestRamps("Ramp_SolveMinAccel_failure.dat");
  printf("Testing ParabolicRamp min-time failures...\n");
  TestRamps("PP_SolveMinTime_failure.dat");
  TestRamps("PLP_SolveMinTime_failure.dat");
  TestRamps("Ramp_SolveMinTime_failure.dat");
  printf("Done.\n");
