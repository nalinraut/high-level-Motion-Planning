...
struct KlamptPlannerProgram : public WorldViewProgram
{
  ...
  bool LoadProblem(const char* worldfile,const char* configsfile)
  {
    ...
    start=configs[0];
    goal=configs[1];
    if(start.n != world.robots[0].robot->q.n) {
      printf("Incorrect size on start configuration\n");
      return false;
    }
    if(goal.n != world.robots[0].robot->q.n) {
  ...
  virtual void RenderWorld() {
	...
	for(roadmap.roadmap.Begin(i,e);!e.end();e++) {
	  Real dt = 0.2;
	  Real len = cspace->Distance((*e)->Start(),(*e)->Goal());
	  //draw end effector path
...
{
  if(argc <= 2) {
    printf("USAGE: KlamptPlan [options] world_file configs_file\n");
    printf("OPTIONS:\n");
  ...
  if(i+2 < argc) {
    ...
    printf("USAGE: KlamptPlan [options] worldfile configsfile\n");
    return 1;
  }
  if(i+2 > argc) {
  ...
  if(collisionMargin > 0) {
    for(size_t i=0;i<program.world.robots[0].robot->geometry.size();i++)
      program.world.robots[0].robot->geometry[i].margin += collisionMargin;
  }
  if(visualize) {
