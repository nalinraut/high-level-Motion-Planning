...
{
  string type;
  if(e->QueryStringAttribute("type",&type)!=TIXML_SUCCESS) {
  ...
  if(type == "prm")
    ...
    factory.type = MotionPlannerFactory::SBLPRT;
  else if(type == "est")
    factory.type = MotionPlannerFactory::EST;
  else if(type == "perturbation")
    factory.type = MotionPlannerFactory::PerturbationTree;
  else if(type == "lazyprm")
    factory.type = MotionPlannerFactory::LazyPRM;
  else {
    printf("Unsupported motion planner type\n");
    return false;
  }
  e->QueryValueAttribute("knn",&factory.knn);
  e->QueryValueAttribute("connectionThreshold",&factory.connectionThreshold);
  e->QueryValueAttribute("ignoreConnectedComponents",&factory.ignoreConnectedComponents);
  e->QueryValueAttribute("perturbationRadius",&factory.perturbationRadius);
  e->QueryValueAttribute("perturbationIters",&factory.perturbationIters);
  e->QueryValueAttribute("bidirectional",&factory.bidirectional);
  e->QueryValueAttribute("useGrid",&factory.useGrid);
  e->QueryValueAttribute("gridResolution",&factory.gridResolution);
  e->QueryValueAttribute("randomizeFrequency",&factory.randomizeFrequency);
  return true;
}
...
bool XmlParse(TiXmlElement* e,RigidRobot2DCSpace& cspace)
{
  ...
  Real defaultAngleWeight = cspace.angleDistanceWeight;
  if(e->QueryValueAttribute("angleWeight",&cspace.angleDistanceWeight)!=TIXML_SUCCESS) {
    cspace.angleDistanceWeight = defaultAngleWeight;
  }
  TiXmlElement* edomain = e->FirstChildElement("domain");  
  if(edomain) {
    if(!XmlParse(edomain,cspace.domain)) return false;
...
{
  bool allowRotation = cspace.allowRotation;
  if(e->QueryValueAttribute("allowRotation",&cspace.allowRotation)!=TIXML_SUCCESS) {
