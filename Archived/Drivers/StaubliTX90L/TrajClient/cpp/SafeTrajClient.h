...
class SafeTrajClient
{
 public:
   ...
   * maximum speed and acceleration is multiplied by rate.
   */
  bool MoveToImmediate(double dt,const Vector& q,double rate=1.0);
   ...
   * in a loop.  It will return an approximate amount of time to sleep, if
   * this is awaiting the robot's motion queue to clear out to allow future
   * commands.
   *
...
bool SafeTrajClient::CheckTrajectory(double tstart,TrajectoryFunction f,double tmax) 
{
  ...
  if(tstart <= 0) {  //add to end of motion queue
    mirror.GetEndConfig(qprev);
    mirror.GetEndVelocity(dqprev);
  }
  else {
  ...
  */
  ...
  while (t < tmax) {
    ...
    if(!this->CheckState(q,dq,ddq)) {
      fprintf(stderr,"CheckPath: State infeasible at time %g\n",t);
...
bool SafeTrajClient::SendTrajectory(double tstart,TrajectoryFunction f,double tmax,double dt)
{
  if(tstart != 0) {
    ...
    if(!ResetTrajectoryAbs(tstart)) return false;
  }
