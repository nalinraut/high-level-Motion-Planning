...
bool TrajServerEmulator::PollRecv(int c,std::vector<std::string>& messages)
{
  ...
  for(int i=0;i<n;i++)
    if(buffer[i] != 0)
      this->partialInput += buffer[i];

  int last = 0;
  for(size_t i=0;i<partialInput.length();i++) {
    if (partialInput[i] == ';') {
      string msg=this->partialInput.substr(last,i-last+1);
      messages.push_back(msg);
      last = i+1;
    }
  }
  this->partialInput = this->partialInput.substr(last,this->partialInput.length()-last);
  return true;
}
...
std::string TrajServerEmulator::DispatchCommand(const std::string& cmd,const std::string& args)
{
  if(cmd == "echo") return args;
    ...
    Vector j;
    if(!ReadVector(args,j)) return "Error";
