...
class SafeTrajClient:
    ...
    def movehermite(self,ts,qs,vs):
        """Produces a hermite move through the times in ts, configurations in
        qs, and tangents in vs,
        """
        self.checkTrajectory(ts,qs,vs)
        self.__addTrajectory(ts,qs,vs)

        
    def addTrajectory(self,f,tmax,dt):
        """Sends the trajectory f(t), for t in [0,tmax], to the
        controller using the discretization dt.  If f(t) is not safe, then an
        exception is raised."""

        n = math.ceil(tmax/dt)
        ts = [i*dt for i in xrange(n+1)]
        qs = [f(t) for t in ts]+[f(tmax)]
        vs = [[0.0]*len(qs[0])]
        for i in xrange(1,n):
            den = ts[i+1]-ts[i-1]
            vs.append([(q2-q1)/den for(q2,q1) in zip(qs[i+1],qs[i-1])])
        vs.append([0.0]*len(qs[0]))
        self.checkTrajectory(ts,qs,vs)
        self.__addTrajectory(ts,qs,vs)
    ...
    def checkTrajectory(self,ts,qs,vs):
        if any(t < 0 for t in ts):
            raise RuntimeError("Negative timing")
        for (t1,t2) in zip(ts[:-1],ts[1:]):
            if t1 > t2:
                raise RuntimeError("Invalid timing")
        milestones = zip(ts,qs,vs)
        for m in milestones:
            self.checkState(m[1],m[2])
        for (m1,m2) in zip(m[:-1],m[1:]):
            ...
            cv1b = 2.0
            cv2b = 4.0
            #get hermite acceleration
            a1 = [0.0]*len(m1[1])
            a2 = [0.0]*len(m1[1])
            for (i,(q1,q2,v1,v2)) in enumerate(zip(m1[1],m2[1],m1[2],m2[2])):
                a1[i] = cq1a*(q1-q2)+cv1a*v1+cv2a*v2
                a2[i] = cq1b*(q1-q2)+cv1b*v1+cv2b*v2
            self.checkState(m1[1],m1[2],a1)
            self.checkState(m2[1],m2[2],a2)


    def __addTrajectory(self,ts,qs,vs):
        """Sends the trajectory f(t), for t in [0,tmax], to the
        ...
        *The caller is responsible for ensuring that the trajectory is safe!*
        ...
        while i < len(qs):
            ...
            self.__t.appendMilestonesQuiet(ts[i:imax+1],qs[i:imax+1],vs[i:imax+1])
            i = imax+1
