...
(250Hz).  MQ(t) interpolates smoothly between *milestones* which are
(time,configuration,velocity) tuples (t[1],q[1],v[1]),...,(t[n],q[n],v[n]).

Interpolation between milestones is performed using a cubic Hermite spline.

A motion queue has a maximum size, which can be queried (command 'gms'). 
If the motion queue is empty, the robot remains in the currently commanded
...
Time is always measured with respect to the absolute time at which the
trajServer3 program is run and motion commands are given.
...
Step 1.
  Launch SRS and the CS8 controller.  Copy the trajServer3 program to the
...
Step 1.
  Launch the CS8 controller if not already running.  Open the trajServer3
...
There are currently a maximum of 200 cubic segments in the trajectory queue.
The CS8 controller can handle "am" commands at approximately 250Hz (perhaps
a bit less).  Usually, however, you would not want to send at the maximum
rate, because the controller would be unresponsive during the motion.  In
our experiments 50Hz is a good tradeoff between accuracy and responsiveness.
...
am(t q v): Appends the milestone q,v at time t.  t must be after the current
           end time.
rtabs(t): Resets the trajectory at time t.  t must be after the current time.
