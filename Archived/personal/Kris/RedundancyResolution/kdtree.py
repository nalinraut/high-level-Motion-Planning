...
class KDTree:
    ...
    def rebalance(self,force=False):
        ...
        def recurse_add_points(points,node):
            points += node.points
            if node.left: recurse_add_points(points,node.left)
            if node.right: recurse_add_points(points,node.right)
        points = []
        recurse_add_points(points,self.root)
        self.set(*zip(*points))
        print "Done."
        return True
