...
if pkg_resources.get_distribution("klampt").version >= '0.7':
	#Klampt v0.7.x
	from klampt import *
	from klampt.model import ik,trajectory
...
class RedundancyResolutionGraph:
	...
	def buildNearestNeighbors(self):
		...
		if len(bmax) == 6:
			self.nnw = NearestNeighbors(workspace_distance,'se3balltree')
		else:
			self.nnw = NearestNeighbors(L2Metric,'kdtree')
	...
	def validEdgeLinear(self,qa,qb,wa,wb,epsilon=1e-2):
		...
		Ndivs = int(math.ceil(self.robot.distance(qa,qb)/epsilon))
		#print Ndivs,"divisions, epsilon",epsilon,"discontinuity threshold",discontinuityThreshold
	...
	def autoSetWorkspaceRadius(self):
		...
		for i,d in self.Gw.nodes_iter(data=True):
			...
			if self.wRadius == oldw:
				...
				numSame = 1
		print "Workspace radius auto-set to",self.wRadius
		self.wRadius *= 1.01
	...
	def interpolate(self,x,r='auto',multi=False):
		...
		if len(nw) == 0:
			...
			if res is None:
				...
				return [q0]
			xn,n = res
			print "Nearest neighbor has distance",workspace_distance(x,xn)
			nw = [(xn,n)]
		if len(nw) == 1:
		...
		if maxd < edgeTol and hasattr(closest,'__iter__'):
			return [closest]
			#return [self.interpolateEdgeBisection(self.Gw.node[iw]['config'],self.Gw.node[jw]['config'],xi,xj,u)]
