...
class RedundancyProgram(GLRedundancyProgram):
	...
	def keyboardfunc(self,c,x,y):
		...
		if c == ' ':
			...
			Nq = raw_input("How many samples per workspace node (Nq, default 10)? > ")
			try:
				Nq = int(Nq)
			except:
				Nq = 10
			self.rr.sampleConfigurationSpace(Nq)
		elif c == 'Q':
			print "Sampling configuration space (baised)..."
			Nq = raw_input("How many samples per workspace node (Nq, default 10)? > ")
			try:
				Nq = int(Nq)
			except:
				Nq = 10
			self.rr.sampleConfigurationSpace(Nq,biased=True)
		elif c == 'c':
			...
			print "Optimizing..."
			iters = raw_input("How many iterations? (default 10) > ")
			try:
				iters = int(iters)
			except:
				iters = 10
			self.rr.optimize(iters)
			self.rr.printStats()
			self.roadmapDisplayList.markChanged()
			self.disconnectionDisplayList.markChanged()
		elif c == 'v':
			...
			print "Using Nq=",self.Nq,"SMM k=",self.visK
...
if __name__ == "__main__":
	import sys
	if len(sys.argv) == 1:
		print "Usage: redundancy_cache.py problem [json file] [opt1=arg1] [opt2=arg2] ..."
