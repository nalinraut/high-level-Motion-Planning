		...
		if self.drawJointLimitColors:
			...
			for i in xrange(self.robot.numLinks()):
				...
				if self.robot.getJointType(i) == 'normal':
					...
					u = 1.0
					u = min(1,(testq[i]-a)/(self.jointLimitColoringRatio*(b-a)))
					u = min(1,(b-testq[i])/(self.jointLimitColoringRatio*(b-a)))
					if u != 1:
						u = max(u,0.0)
						c = vectorops.add(vectorops.mul(oldColors[i],u),vectorops.mul(self.jointLimitColor,1-u))
	...
	def idle(self):
		...
		if self.walk_path != None:
			...
			if self.walk_progress >= self.walk_path.times[-1]:
				...
				qstraight = self.resolution.solve(self.temp_config,vectorops.interpolate(self.walk_workspace_path.milestones[0],self.walk_workspace_path.milestones[-1],u),testfeasibility=False)
