...
class Database:
    ...
    def readCSV(self,fn,maxentries=None,storage=None,dtype=None):
        """Reads up to maxentries entries in a CSV file with header.
        If storage==None, each entry will be a list
        If storage=='numpy', this will cast each entry to a numpy array.

        dtype may be a type, like int or float. If so, each element will
        ...
        for lineno,line in enumerate(reader):
            if lineno == 0:
                ...
                if storage == 'numpy':
                    ...
                    if dtype != None:
                        line = [dtype(v) for v in line]
                self.entries.append(line)
            if maxentries is not None and lineno >= maxentries:
                break
        f.close()
    ...
    def cast_numpy(self,type=None):
        """Casts each entry to a Numpy array."""
        import numpy as np
        if type == None:
            type = np.float_
        for i,e in enumerate(self.entries):
            self.entries[i] = np.array(e,dtype=type)
                ...
                if isinstance(v,int):
                    ...
                    try:
                        ...
                        try:
                            ...
                            return str
            if ints == len(vals):
                #print "Key",key,"is determined to be int"
                return int
            elif ints + floats == len(vals):
                #print "Key",key,"is determined to be float"
                return float
    ...
    def get_if(self,cond,data={}):
        ...
        for e in enumerate(self.entries):
            ...
            if eval(cond,globals(),locals):
                db.entries.append(e)
        return db
    def get_column(self,key_or_index):
        """Returns a list containing each of the entries' values
        corresponding to the given column, where the column is indexed
        by key (str), or integer index (int)"""
        if key_or_index in self.keys:
            index = self.keys.index(key_or_index)
        else:
            index = key_or_index
        return [e[index] for e in self.entries]
    def get_columns(self,keys_or_indices):
        """Returns a list of lists containing each of the entries' values
        corresponding to the given columns, where each column is indexed
        by key (str), or integer index (int)"""
        indices = []
        for ki in keys_or_indices:
            if ki in self.keys:
                indices.append(self.keys.index(ki))
            else:
                indices.append(ki)
        return [[e[index] for index in indices] for e in self.entries]
