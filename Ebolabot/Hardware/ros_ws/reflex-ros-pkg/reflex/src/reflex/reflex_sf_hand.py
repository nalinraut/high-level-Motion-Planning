...
class ReflexSFHand(ReflexHand):
    def __init__(self,name):
        super(ReflexSFHand, self).__init__('/reflex_'+name, ReflexSFMotor)
	...
	for motor in self.motors:
	    self.motors[motor].update_moving_hook=lambda data:self.moving_flag(data)
    ...
    def enable_torque(self):
        for ID, motor in self.motors.items():
            motor.enable_torque()
    def moving_flag(self,data):
	self.is_moving=data.is_moving or self.is_moving
...
motor, or 'q' to indicate that the zero point has been reached\n")
	...
	for item in ['_f1','_f2','_f3','_preshape']:
	    data[self.prefix+item]=dict(zero_point=self.motors[self.namespace+item].get_current_raw_motor_angle())
        self._write_zero_point_data_to_file(self.prefix+'_zero_points.yaml', data)
...
if __name__ == '__main__':
    import sys
    if len(sys.argv)<=1:
        print "USAGE: rosrun reflex reflex_sf_hand.py NAME"
        exit(0)
    main(sys.argv[1])
