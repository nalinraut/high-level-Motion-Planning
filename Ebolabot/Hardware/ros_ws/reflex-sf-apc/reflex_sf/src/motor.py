...
class Motor(object):
    def __init__(self, name):
        ...
        self.overload_threshold=rospy.get_param(self.name+'/overload_threshold')
	self.calibration_threshold=rospy.get_param(self.name+'/calibration_threshold')
        self.calibration_count=rospy.get_param(self.name+'/calibration_count')
        self.calibration_flip=rospy.get_param(self.name+'/calibration_flip')
	...
	#print self.name,'motor load:',data.load,'\n'
 	load_filter=0.25
	self.load=load_filter*data.load+self.load*(1-load_filter)
	if self.load<self.overload_threshold:
	    self.loosen()
	    print self.name,'load:',self.load,'loosing\n'
