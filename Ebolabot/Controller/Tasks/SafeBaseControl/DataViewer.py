from BaseSensing import Base, Lidar, Filter
...
lidar_protection = 35 # (mm) radius from lidar at which collision is assumed
...
while not connected:
    try:
        ...
        laser1 = Lidar(1, base_x + (-base.w / 2) + lidar_offset, base_y + (base.h / 2) - lidar_offset, 180-45, True, lidar_protection)       # top left (actual)
        # laser1 = Lidar(1, base_x - (base_w / 2) - lidar_offset, base_y + (base_h / 2) + lidar_offset,  135, True, lidar_protection)
        ...
        # laser1 = Lidar(1, base_x + (base_w / 2) + lidar_offset, base_y - (base_h / 2) - lidar_offset, 315, True, lidar_protection) # bottom right corner (actual 2)
        print 'Lidar 1 connected'
    except:
    ...
    try:
        laser2 = Lidar(2, base_x + (base_w / 2) - lidar_offset, base_y - (base_h / 2) + lidar_offset, -45, True, lidar_protection) # bottom right corner (actual 2)
        print 'Lidar 2 connected'
        lasers = [laser1, laser2]

    except:
        print 'Lidar 2 not connected'
        time.sleep(0.2)
        continue
    connected = True
...
# while 1:
    ...
    base.plot_base(ax1)
    for laser in lasers:
        ax1.plot(laser.x_pos, laser.y_pos, 'y.', markersize=laser.collision_radius, alpha=0.3)  # Location of Lidar
    ...
    # Plot data
    for laser in lasers:
        ax1.plot(laser.x, laser.y, 'b.', markersize=2) # All points
    # ax1.plot(lasers[0].x, lasers[0].y, 'b.', markersize=2) # All points laser 1
    # ax1.plot(lasers[1].x, lasers[1].y, 'g.', markersize=2)  # All points laser 2
    ...
    # filter.classify_points()
    # for i in range(len(laser1.x)):
    #     if filter.classes[i][0] == 'obstacle':
    #         ax1.plot(laser1.x[i], laser1.y[i], 'b.', markersize = 2)
    #     elif filter.classes[i][0] == 'tether':
    #         if filter.classes[i][1] == 0:
    #             ax1.plot(laser1.x[i], laser1.y[i], 'g.', markersize=4)
    #         elif filter.classes[i][1] == 1:
    #             ax1.plot(laser1.x[i], laser1.y[i], 'y.', markersize=2)
    # for tether in filter.tethers:
    #     ax1.plot([tether[0], tether[2]], [tether[1], tether[3]], 'y:', markersize=4)

    # time code:
    times[3] += time.time() - startTime
    print '%f sec to %s' % (times[3] / time_counter, 'filter data')
    startTime = time.time()

    # Find collision Points
    # t_to_collision, collision_point = base.time_to_collision([laser1])

    # time code:
    times[4] += time.time() - startTime
    print '%f sec to %s' % (times[4] / time_counter, 'search for collision')
    startTime = time.time()
    ...
    print '%f sec to %s' % (times[5] / time_counter, 'plot collision trajectory')
    startTime = time.time()

    # Find control factor and plot
    # cf = base.get_control_factor(t_to_collision, collision_point)
    # print 'Control factor:', cf, '\tTTC:', t_to_collision
    ...
    print '%f sec to %s' % (times[6] / time_counter, 'find control factors')
    startTime = time.time()
    ...
    # time code:
    times[7] += time.time() - startTime
    print '%f sec to %s' % (times[7] / time_counter, 'reconfigure axes')
