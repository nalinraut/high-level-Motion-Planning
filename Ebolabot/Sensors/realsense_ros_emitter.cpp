...
//==========================================================================================================================================
  ...
  double scale = 0.001;
  float FOVy = (double)msg["FOVx"]*pi/180;
  float FOVx = (double)msg["FOVy"]*pi/180;
  //float FOVx = 72*pi/180.0;
  //float FOVy = 40*pi/180.0;
  float xMultiplier = 1.0/(colorWidth*tan(FOVx/2));
  float yMultiplier = 1.0/(colorHeight*tan(FOVy/2)); 
  ...
  if(png_decomp){
    for(int j=0;j<ros_pc_msg->height;j++) {
      for(int i=0;i<ros_pc_msg->width;i++,k++) {
        ...
        float z = 0;
        if(raw_depth){
          z = (ros_depth_msg->data[2*(i+j*depthWidth)])*scale;   
        }else{
          z = (depth_buffer[i+j*ros_pc_msg->width])*scale;   
        }
