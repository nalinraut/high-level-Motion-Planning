...
bool Loop::ProcessPC(AnyCollection& msg,File& datastream)
{
  ...
  sensor_msgs::CompressedImage* ros_depth_msg = NULL;
  string name;
  ...
  */
  ...
  ros_depth_msg = &compressed_depth_msg;
  ...
  if(format != "jpg,png,uvX,uvY" && format != "jpg,raw,uvX,uvY") {
    printf("TODO: parse non fffccc or sssccc format point clouds\n");
    return false;
  }
  ...
  //printf("about to start reading data\n");

  ros_rgb_msg->data.resize(size1);

  // int *uvXBuf = new int[size3/sizeof(int)];
  // int *uvYBuf = new int[size4/sizeof(int)];
  vector<char> uvXBuf, uvYBuf;
  uvXBuf.resize(size3);
  uvYBuf.resize(size4);


  uvX.resize(2*ros_pc_msg->height*ros_pc_msg->width);
  uvY.resize(2*ros_pc_msg->height*ros_pc_msg->width);
  
  //printf("attempting to save data to file\n");

  //std::ofstream dummyFileSave("dummyPic.jpg", std::ios::binary);
  ...
  while(cnt < size1) {
    ...
    if(buf.length() != packet_size && cnt+buf.length() != size1) {
      printf("Warning, abnormal buffer size %d\n",(int)buf.length());
    }
    if(cnt + buf.length() > size1) {
      printf("Weird, sum of streamed message lengths is too large: %d vs %d\n",cnt+(int)buf.length(),size1);
      printf("Buf size: %d\n",(int)buf.length());
      return false;
    }
    copy(buf.begin(),buf.end(),&ros_rgb_msg->data[cnt]);
    cnt += buf.length();
    //dummyFileSave.write(&buf[0], buf.length());
  
  }
  bytes_read += cnt;
  
  //printf("got color data\n");
  ...
  while(cnt < size2) {
    ...
    if(buf.length() != packet_size && cnt+buf.length() != size2) {
      printf("Warning, abnormal buffer size %d\n",(int)buf.length());
    }
    if(cnt + buf.length() > size2) {
      printf("Weird, sum of streamed message lengths is too large: %d vs %d\n",cnt+(int)buf.length(),size2);
      printf("Buf size: %d\n",(int)buf.length());
      return false;
    }
    copy(buf.begin(),buf.end(),&ros_depth_msg->data[cnt]);
    cnt += buf.length();
  }
  bytes_read += cnt;
  ...
  while(cnt < size3) {
    ...
    //printf("after reading string\n");
    if(buf.length() != packet_size && cnt+buf.length() != size3) {
      printf("Warning, abnormal buffer size %d\n",(int)buf.length());
    }
    if(cnt + buf.length() > size3) {
      printf("Weird, sum of streamed message lengths is too large: %d vs %d\n",cnt+(int)buf.length(),size3);
      printf("Buf size: %d\n",(int)buf.length());
      return false;
    }
    //printf("one iteration\n");
    copy(buf.begin(),buf.end(),&uvXBuf[cnt]);
  ...
  while(cnt < size4) {
    ...
    if(buf.length() != packet_size && cnt+buf.length() != size4) {
      printf("Warning, abnormal buffer size %d\n",(int)buf.length());
    }
    if(cnt + buf.length() > size4) {
      printf("Weird, sum of streamed message lengths is too large: %d vs %d\n",cnt+(int)buf.length(),size4);
      printf("Buf size: %d\n",(int)buf.length());
      return false;
    }
    copy(buf.begin(),buf.end(),&uvYBuf[cnt]);
    cnt += buf.length();
  }
  bytes_read += cnt;
  // got the data
  ...
  // decompressPNG(&ros_depth_msg->data[0]); // goes to depth_buffer
  // //ros_rgb_msg -> data // contains jpg of image
  //ros_depth_msg -> data // contains png of image
  ...
  for(int i = 0; i< (int)(size3/sizeof(int)); i+=2 ){
    ...
    //printf("duration: %d\tval:%d\n", duration, val);
  }
  
  //printf("done reconstructing\n");
  ...
  FILE *f;
  f = fopen("pointcloud.txt", "w");
  fprintf(f, "Start\n");

  
  for(int i=0;i<ros_pc_msg->width;i++) {
    for(int j=0;j<ros_pc_msg->height;j++,k++) {
      //copy the k'th pixel

      unsigned char* outdata = &ros_pc_msg->data[ros_pc_msg->point_step*k];

      //tangent is radians
      float z = (depth_buffer[i+j*ros_pc_msg->width])*scale;
      float x= -2.0*(j-cx)/colorWidth*tan(FOVx/2)*(z);
      float y = 2.0*(i-cy)/colorHeight*tan(FOVy/2)*(z);
      
      int colorI = uvX[i*ros_pc_msg->height+j];
      int colorJ = uvY[i+ros_pc_msg->width*j];
      ...
      *(unsigned int*)(outdata+12) = rgb;
      ...
      //fprintf(f, "x: %f, y: %f, z: %f, r: %d, g: %d, b: %d\n", x, y, z, r,g,b);
  ...
  //printf("file done\n");
  //printf("K is %d\n", k);
  //printf("Total possible K is: %d\n", ros_pc_msg->width*ros_pc_msg->height);
  // delete(uvXBuf);
  // delete(uvYBuf);
  free(bmp_buffer);
  bmp_buffer = NULL;
